...
-- Find the player name and the player class having an inventory that contains a weapon that has damage type 'bludgeoning'.
SELECT 	P.playerName, P.class, I.inventoryName, W.wName
...
WHERE 	p.playerName = I.iPlayer AND
	...
	W.wid = D.wid AND
	D.damageType = 'bludgeoning';
-- < Q02 - A self-join > 
-- Find the pair of players having at least one of the same inventory. 
SELECT	I1.iPlayer, I2.iPlayer, I1.inventoryName
...
-- Find the total weight of weapons in each inventory.
SELECT	I.iPlayer, I.inventoryName, SUM(W.wWeight)
FROM 	weapon W, inventory I
WHERE	I.iPlayer = W.playerName AND
	I.inventoryName = W.inventoryName
GROUP BY I.iPlayer, I.inventoryName;
-- < Q05 - GROUP BY, HAVING, and ORDER BY, all appearing in the same query. >
-- Find all player inventories that have more than 1 weapon in the inventory,
-- also find the total weight of weapons the inventory is holding and sort it by total weight.
SELECT  I.iPlayer, I.inventoryName, SUM(W.wWeight), COUNT(*)
FROM    weapon W, inventory I
WHERE   I.iPlayer = W.playerName AND
        I.inventoryName = W.inventoryName
GROUP BY I.iPlayer, I.inventoryName
HAVING 	COUNT(*) > 1
ORDER BY SUM(W.wWeight);
-- < Q06 - A correlated subquery. >
-- -- Find the name of every weapon with the damage type 'slashing'
SELECT W.wName
FROM weapon W
WHERE EXISTS
	(SELECT D.wid
...
-- < Q08a - A complicated but ingeniously devised query >
...
FROM weapon;
-- < Q11 - A Top-N query. >
-- Finding the top three casterlevel of spell in the inventory.
